...
def load_query_df() -> pl.DataFrame:
    ...
    query_df = (
        query_df.filter(~pl.col("visit_id").is_in(MY_VISIT_IDS))
        # remove URL encoding (e.g., "%20" -> " ")
        .with_columns(
            pl.col("query").map_elements(lambda x: unquote(x), return_dtype=pl.String)
        ).with_columns(
            lowercase_and_strip("query").alias("query")
...
def calculate_relevance_df(
    ...
    """
    return (
        query_df.with_columns(lowercase_and_strip("item_name").alias("item_name"))
        .with_columns(
            pl.when(pl.col("search_index") > max_i)
            .then(max_i)
        ...
        .with_columns(
            (pl.col("score") / pl.max("score").over("query")).alias("normalized_score")
        )
...
def collect_relevance_df(
    ...
    if level == "name":
        ...
        item_df = (
            ...
            .with_columns(
                pl.concat_str("item_number", pl.lit("-"), "item_name").alias(
                    "item_number"
                )
            )
        )
    ...
    return relevance_df.group_by("query").agg(
...
def get_retrieved_df(
    ...
    if search_type == "semantic search" and semantic_search is None:
        raise ValueError(
            "Must provide `semantic_search` if `search_type` is 'semantic'"
        )
    ...
    for query in query_list:
        ...
        if level == "name":
            ...
            _df = _df.with_columns(
                pl.concat_str("number", pl.lit("-"), "item_name").alias("item_number")
            )
        _df = _df.group_by("query").agg(pl.col(collect_col).alias("retrieved_list"))
        df_list.append(_df)
