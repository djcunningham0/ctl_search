...
with st.expander("relevance parameters"):
    ...
    view_weight = c1.number_input(
        "view weight",
        value=float(DEFAULT_VIEW_WEIGHT),
        min_value=0.0,
        step=0.5,
        key=f"view_weight_{st.session_state['relevance_scores_i']}",
    )
    hold_weight = c2.number_input(
        "hold weight",
        value=float(DEFAULT_HOLD_WEIGHT),
        min_value=0.0,
        step=0.5,
        key=f"hold_weight_{st.session_state['relevance_scores_i']}",
    )
    alpha = c3.number_input(
        "alpha",
        value=DEFAULT_ALPHA,
        min_value=0.0,
        step=0.1,
...
relevance_df = calculate_relevance_df(
    query_df, view_weight=view_weight, hold_weight=hold_weight, alpha=alpha, max_i=max_i
)
